...
export default class RoadMaps extends Component {
  ...
  componentWillMount() {
    ...
    const groups = coordinatesss.map((e, i) => {
      return i % chunkSize === 0 ? coordinatesss.slice(i, i + chunkSize) : null;
    }).filter(e => { return e });
    console.log("Groups", JSON.stringify(groups))
    for (var i = 0; i < groups.length; i++) {

      this.state.coordinatesnew = []
      this.state.coordinatesnew = JSON.stringify(groups[i].join('|'))
      this.calculatePath(this.state.coordinatesnew)
    }
  ...
  calculatePath = (newarrs) => {
    ...
    // ).then(resp => {
    //   var datas = resp.data
    //   console.log("aaaaaaaaaaaaaaaaaaaaaaaaa",JSON.stringify(datas))
  ...
  calculateDistance = () => {
    var dest1 = 0;
    console.log("in calc....");
    finalArray = this.state.finalCoords
    console.log("RAjaniDAta=====", JSON.stringify(this.state.finalCoords))
    console.log("TimeDist bmbmbm")
    var dist = 0
    ...
    for (var i = 0; i < this.state.finalCoords.length; i++) {
      ...
      console.log("new100001=", dist)
      dest1 = dest1 + dist
      //dest1 += dist
      console.log("newDistance10000=", dest1)
    }
    var Km=0
    var meter=0
     Km = Math.floor(dest1 / 1000)
    meter = dest1 % 1000
    // Km = Number(Km.toFixed(0));
    // meter = Number(meter.toFixed(0));
    //  console.log("TimeDistanceKminnnnnnnnnn=", Km, meter)
    meter = Number(meter).toFixed(0);
    this.setState({ Km: Km });
    ...
    //   try {
    //     dist11 = haversines.distance(coordinatesss[i], coordinatesss[i + 1])
    //   } catch (err) {
    //     // handle error here
    //   }
    //   // dist=Math.round(dist,2)
    //   //dist11 = Number(dist11.toFixed(2));
    //   //console.log("distancr1=", dist11)
    //   dest12 += dist11
    //   console.log("newDistance=", dest12)
    //   var Km = Math.floor(dest12 / 1000)
    //   var meter = dest12 % 1000
    //   Km = Number(Km.toFixed(0));
    //   meter = Number(meter.toFixed(0));
    //   console.log("TimeDistanceKmnew...............=", Km, meter)
    //   this.setState({ Km: Km });
    //   this.setState({ meter: meter });    
